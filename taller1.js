...
calcularDistancia(7,5,3,8, function(parametro){
...
// Ejercicio 2 - funcion flecha
obtenerNombre = codigo1 => nombre = codigo.split(':')[1] + codigo.split(':')[0]
let resultado = obtenerNombre('11012345 : Gilma') 
console.log(resultado)
...
function monitorearTemperatura(grados){
    let tempPromedio
    let sumaTempPorDia = 0
    let cantidadesTemp=0

    grados.forEach(function(grado){
        sumaTempPorDia=(sumaTempPorDia+grado)
        cantidadesTemp = cantidadesTemp +1
        tempPromedio = sumaTempPorDia/cantidadesTemp
    })
    console.log('Segun el numero de ' +cantidadesTemp + ' días ingresados, la temperatura promedio es: ' +  tempPromedio.toFixed() + ' grados')
}
...
let monitorearTemperatura = grados => tempPromedio, sumaTempPorDia = 0 , cantidadesTemp=0

    grados.forEach = grado => sumaTempPorDia=(sumaTempPorDia+grado), cantidadesTemp = cantidadesTemp +1  , tempPromedio = sumaTempPorDia/cantidadesTemp
...
revisarSables(arregloSables, function(filtrarSables){
    console.log( 'los sables negativos fueron: ' + filtrarSables + ' ----//----  la cantidad de sables con energia negativa es: ' +filtrarSables.length)
})
...
function principal (nombre, planeta, edad, estatura,secundaria){
    ...
    principal('juan', 'tierra', 16, 1.60, function(parametro){  
        if (parametro.edad > 15){
            console.log(parametro.datosCompletos() + parametro + "Manejo de la fuerza")
        } else{
            console.log(parametro.datosCompletos() + parametro + 'Manejo del sable de luz')
        }
    })


// Ejercicio 6 - funcion callback
